...
const saveAllData = async () => {
  ...
  const submitVote = async (votingType, voteData) => {
    console.log('🗳️ Guardando voto...');
    console.log('👤 Usuario:', currentUser);
    console.log('📊 Tipo:', votingType);
    console.log('🎯 Datos:', voteData);
    ...
    // Verificar que hay votación activa
    if (!activeVoting || !votingStartTime) {
      alert('No hay votación activa en este momento');
      return;
    }
    
    // Calcular tiempo transcurrido
    const timeElapsed = (Date.now() - votingStartTime) / 1000; // en segundos
    const timeElapsedMinutes = timeElapsed / 60;
    
    // Si pasaron más de 3 minutos, no permitir
    if (timeElapsedMinutes > 3) {
      alert('El tiempo para votar ha expirado (máximo 3 minutos)');
      return;
    }
    
    // Verificar si ya existe un voto locked para este usuario/votingType
    const docId = `${currentUser}_${votingType}`;
    const voteRef = doc(db, 'votes', docId);
    
    try {
      const existingDoc = await getDoc(voteRef);
      if (existingDoc.exists() && existingDoc.data().locked === true) {
        alert('⚠️ Este voto ya fue enviado y no puede ser editado');
        return;
      }
    } catch (error) {
      console.log('No existe voto previo, continuando...');
    }
    ...
    try {
      ...
      const voteDocument = {
        ...
        activeVotingSession: activeVoting // Para tracking
      };
      
      console.log('💾 Guardando voto en Firestore...');
      console.log('📄 Doc ID:', docId);
      console.log('🔒 Locked: true (no se puede editar)');
      
      // GUARDAR en Firestore
      await setDoc(voteRef, voteDocument);
      
      console.log('✅ Voto guardado en Firestore exitosamente');
      ...
      alert('✅ Voto enviado exitosamente y bloqueado para edición');
      
      // Recargar votos (implementar si es necesario)
